...
settlers_of_cryptotan_service_file:
  file.managed:
    ...
    - contents: |
        ...
        DynamicUser=yes
        Group=www-data
        ExecStart=/usr/local/bin/uwsgi --python-path /usr/lib/python3/dist-packages --python-path /usr/local/lib/python3.5/dist-packages --no-site -L -p 5 --limit-post 131072 --master --wsgi-file /usr/local/lib/python3.5/dist-packages/settlers_of_cryptotan.py --callable __hug_wsgi__ --uwsgi-socket /run/settlers_of_cryptotan/uwsgi.sock --need-app --chmod-socket=660 --http-socket [::1]:3000 --http-socket 127.0.0.1:3000
        [Install]
...
settlers_of_cryptotan_service_running:
  service.running:
    ...
    - enable: True
    - watch:
      - file: /usr/local/lib/python3.5/dist-packages/settlers_of_cryptotan.py
