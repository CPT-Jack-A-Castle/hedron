...
hedron_vmmanagement_baremetal_ssh_library:
  file.managed:
    - name: {{ grains['hedron.python.dist.path'] }}/vmmanagement_client_ssh.py
...
hedron_vmmanagement_baremetal_library:
  file.managed:
    - name: {{ grains['hedron.python.dist.path'] }}/vmmanagement_baremetal.py
...
hedron_vmmanagement_baremetal_service_file:
  file.managed:
    ...
    - contents: |
        ...
        Group=vmmanagement
        User=vmmanagement
        ExecStart=/usr/local/bin/uwsgi --python-path /usr/lib/python3/dist-packages --python-path {{ grains['hedron.python.dist.path'] }} -L -p 5 --limit-post 131072 --master --wsgi-file {{ grains['hedron.python.dist.path'] }}/vmmanagement_baremetal.py --callable __hug_wsgi__ --uwsgi-socket /run/vmmanagement_baremetal/uwsgi.sock --need-app --chmod-socket=660
        NoNewPrivileges=yes
        PrivateDevices=yes
        ProtectSystem=full
        [Install]
...
hedron_vmmanagement_baremetal_service_running:
  service.running:
    ...
    - enable: True
    - watch:
      - file: {{ grains['hedron.python.dist.path'] }}/vmmanagement_baremetal.py
